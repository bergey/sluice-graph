    ...
    .controller('BuildingList', ['$scope', '$http', 
                                 function($scope, $http) {
                                     $http.get('Benchmarking_Data_Public.json').success(function(data) {
                                         ...
                                         $scope.marginTop = 25;
                                         var x = d3.scale.linear().range([0,$scope.width-$scope.marginLeft-$scope.marginRight]).domain([$scope.sfMin, $scope.sfMax]);
                                         var y = d3.scale.linear().range([$scope.height-$scope.marginTop-$scope.marginBottom,0]).domain(d3.extent(data, _.property("SITE_EUI")));
